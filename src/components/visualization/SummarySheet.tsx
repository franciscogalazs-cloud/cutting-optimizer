        ...
        <div className="px-4 pb-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm table-fixed">
              ...
              <thead>
                <tr className="bg-[var(--bg)]/50 text-[var(--muted)]">
                  <th scope="col" className="px-3 py-2 text-left w-[72px]">Cant.</th>
                  <th scope="col" className="px-3 py-2 text-left">Detalle</th>
                  <th scope="col" className="px-3 py-2 text-right w-[128px]">Unitario</th>
                  <th scope="col" className="px-3 py-2 text-right w-[128px]">Subtotal</th>
                </tr>
              </thead>
              <tbody>
                {aggItems.map((it, idx) => {
                  ...
                  return (
                    <tr key={idx} className="odd:bg-[var(--bg)]/30">
                      <td className="border-t border-[var(--border)] px-3 py-2 align-top text-left font-mono tabular-nums">{formatCountForItem(it.detalle, Number(it.cantidad) || 0)}</td>
                      {/* Usamos th con scope=row para accesibilidad */}
                      <th scope="row" className="border-t border-[var(--border)] px-3 py-2 text-left font-normal text-[var(--text)] truncate max-w-[1px]">{it.detalle}</th>
                      <td className="border-t border-[var(--border)] px-3 py-2 text-right font-mono tabular-nums">{formatCLP(it.unitario)}</td>
                      <td className="border-t border-[var(--border)] px-3 py-2 text-right font-mono tabular-nums">{formatCLP(sub)}</td>
                    </tr>
                  );
                })}
                ...
                {(() => {
                  ...
                  <tr key={`summary-${i}`} className="odd:bg-[var(--bg)]/30">
                    <td className="border-t border-[var(--border)] px-3 py-2 text-left font-mono tabular-nums">{formatCountForSummary(row.label, row.count as number | string | undefined)}</td>
                    <th
                      scope="row"
                      className={[
                        'border-t border-[var(--border)] px-3 py-2 text-left',
                        row.strong ? 'font-semibold' : 'font-normal',
                      ].join(' ')}
                    >
                      {row.label}
                    </th>
                    <td className="border-t border-[var(--border)] px-3 py-2 text-right font-mono tabular-nums">{formatCLP(row.value)}</td>
                    <td
                      className={[
                        'border-t border-[var(--border)] px-3 py-2 text-right font-mono tabular-nums',
                        row.strong ? 'font-semibold' : 'font-normal',
